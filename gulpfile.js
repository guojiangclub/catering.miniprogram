...
function getFile(type){
    ...
    var filePath = fs.readdirSync(path.resolve('./src'))
    var fileArr = []
    filePath.forEach(function(item){
        if (item == 'pages') {
            var pagePath = path.join('./src',item)
            var pageFiles = fs.readdirSync(pagePath)

            pageFiles.forEach(function(item) {
                 var pagePath = path.join('./src/pages',item)
                 var itemFiles = fs.readdirSync(pagePath)
                itemFiles.forEach(function (v) {
...
gulp.watch('src/component/**/*.less',['less','changeFileName'])
