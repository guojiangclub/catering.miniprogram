...
Page({
    ...
    // 秒杀开始
    isStarts() {
        if (this.data.indexData.seckill.init_status != 1) {
            this.setData({
                'indexData.seckill.init_status': 1
            })
        }
    },
    // 秒杀结束
    isEnd() {
        this.setData({
            'indexData.seckill': null
        })
    },
    // 集call结束
    isFreeEnd() {
        this.setData({
            'indexData.FreeEvent': null
        })
    },
    isFreeStarts() {
        this.setData({
            'indexData.FreeEvent.free.activity_status': 'OnGoing'
        })
    },
    login(){
        pageLogin(getUrl());
    ...
    convertCoupon(code, index) {
        ...
        sandBox.post({
            ...
            if (res.statusCode == 200) {
                ...
                if (res.status) {
                    this.setData({
                        [`indexData.Mini_coupons[${index}].has_get`]: true
                    })
                    /!* var list = `dataList.${activeIndex}`;
                     this.setData({
                         [`${list}[${index}][${findex}].has_get`]: true
                     })*!/
                    wx.showToast({
                        title: '领取成功',
                    });
